...
cfg = ft_checkconfig(cfg, 'renamedval',  {'maplimits',   'absmax',   'maxabs'});
...
cfg = ft_checkconfig(cfg, 'renamed',     {'electrod',    'electrodes'});
cfg = ft_checkconfig(cfg, 'renamed',     {'hcolor',      'headcolor'});
cfg = ft_checkconfig(cfg, 'renamed',     {'electcolor',  'ecolor'});
cfg = ft_checkconfig(cfg, 'renamed',     {'emsize',      'emarkersize'});
cfg = ft_checkconfig(cfg, 'renamed',     {'efsize',      'efontsize'});
...
if strcmp(cfg.electrodes,'on') || strcmp(cfg.showlabels,'markers')
  if ischar(cfg.highlight)
    ft_plot_vector(x,y,'style',cfg.emarker,'Color',cfg.ecolor,'markersize',cfg.emarkersize);
  elseif isnumeric(cfg.highlight)
    normal = setdiff(1:length(x), cfg.highlight);
    ft_plot_vector(x(normal), y(normal), 'style', cfg.emarker, 'Color', cfg.ecolor, 'markersize', cfg.emarkersize);
    ft_plot_vector(x(cfg.highlight), y(cfg.highlight), 'style', cfg.hlmarker, 'Color', cfg.hlcolor, 'markersize', cfg.hlmarkersize, ...
...
elseif any(strcmp(cfg.electrodes,{'highlights','highlight'}))
  if isnumeric(cfg.highlight)
    ft_plot_vector(x(cfg.highlight),y(cfg.highlight),'style',cfg.hlmarker,'Color',cfg.hlcolor,'markersize',cfg.hlmarkersize, 'linewidth',cfg.hllinewidth, 'markerfacecolor', cfg.hlfacecolor);
  else
    error('Unknown highlight type');
  end;
...
elseif strcmp(cfg.electrodes,'numbers') || strcmp(cfg.showlabels,'numbers')
  for i = 1:numChan
    ft_plot_text(x(i), y(i), int2str(i), 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', 'Color', cfg.ecolor, 'FontSize',cfg.efontsize); %%% RD plot_text --> ft_plot_text
  end
...
if isfield(cfg.layout, 'outline')
  ...
  for i=1:length(cfg.layout.outline)
    ft_plot_vector(cfg.layout.outline{i}(:,1), cfg.layout.outline{i}(:,2), 'Color', cfg.headcolor, 'LineWidth', cfg.hlinewidth) %%% RD plot_vector --> ft_plot_vector
  end
end
